...
- **实时通讯**: 与匹配对象进行实时聊天
- **AI匹配顾问**: 匹配后可向 AI 提问"为什么匹配到TA？"或"如何破冰开启话题？"，系统检索匹配数据（子分数、共享/互补特质、双方档案）+ 破冰知识库，由 LLM 生成个性化回答（RAG）
...
- **RAG**: Redis 向量检索 + LLM 生成（匹配解释、破冰建议）
- **迁移**: Flyway (数据库版本管理)
- **构建**: Maven Wrapper
...
- WebSocket维持长连接
- 消息持久化存储
- 聊天页面顶部 "AI匹配顾问" 按钮，展开内嵌问答面板，支持快捷提问和自由输入

### 8. AI匹配顾问（RAG）
- 匹配成功后，聊天页顶部出现 "AI匹配顾问" 按钮
- 用户可提问 "为什么匹配到TA？" 或 "如何破冰？"
- 系统检索匹配数据（6维子分数、共享/互补特质、双方档案）+ 破冰知识库（18条模板，余弦相似度检索Top-3）
- LLM 结合检索结果生成个性化回答
...
| matches | 周匹配记录（含 match_reason JSON 子分数） |
...
| POST | /api/matches/{matchId}/ask | RAG 提问（匹配解释 / 破冰建议） |
...
7. 贪心算法（按平均兼容分排序后配对）
8. 创建 Match 记录，6维子分数写入 matchReason JSON（供 RAG 检索）
```
...
```
rag:doc:icebreaker:{id} = {
  "text": "从对方的个人简介中找共同爱好作为切入点...",
  "embedding": "0.12,-0.34,..."                # 1536维embedding
}
```

启动时自动初始化 18 条中文破冰模板。用户提问时，将问题 embedding 与知识库做余弦相似度匹配，取 Top-3 检索结果注入 LLM 上下文。
